...
interface FilterCategory { id: string; label: string; icon: string; }
interface CarouselSlide   { src: string; alt: string; }
...
export default function TechloHero() {
  ...
  const { query, setQuery, activeFilter, setActiveFilter } = useSearch();

  // ── Carousel-only local state ──
  ...
  // On submit, smooth-scroll down to the product section
  ...
  return (
    <>
      <style>{`
        ...
        .search-glow:focus-within { box-shadow: 0 0 0 1px #00f5e055, 0 0 24px #00f5e022; }
      `}</style>

      <section className="w-full bg-[#080810] pt-6 pb-10 px-4 md:px-10 lg:px-16 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center mb-7 hero-animate" style={{ animationDelay: "0.05s" }}>
          ...
          <h1 className="font-mono font-bold text-2xl md:text-4xl lg:text-5xl text-white leading-tight"
            style={{ textShadow: "0 0 40px #00f5e022" }}>
            Find Your Next{" "}
            <span className="text-[#00f5e0]" style={{ textShadow: "0 0 20px #00f5e077" }}>Gadget</span>
          </h1>
        </div>

        {/* Search bar */}
        <div className="max-w-2xl mx-auto mb-5 hero-animate" style={{ animationDelay: "0.15s" }}>
          <form onSubmit={handleSearch}
            className="search-glow flex items-center bg-[#0d0d1f] border border-[#00f5e033] rounded-2xl overflow-hidden transition-all duration-300">

            {/* Category dropdown — updates context directly */}
            <div className="relative flex-shrink-0">
              <select
                ...
                className="appearance-none h-[52px] pl-4 pr-8 bg-transparent text-[#00f5e0]
                  ...
                  border-r border-[#00f5e022] outline-none focus:bg-[#00f5e00a] transition-colors duration-200"
                aria-label="Filter by category"
              >
                {FILTER_CATEGORIES.map((cat) => (
                  <option key={cat.id} value={cat.id} className="bg-[#0d0d1f] text-[#00f5e0]">
                    {cat.icon} {cat.label}
                  </option>
                ))}
              </select>
            ...
            {/* Text input — live-filters as user types */}
            <input
              type="text"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search phones, laptops, cameras…"
              className="flex-1 h-[52px] px-4 bg-transparent text-white placeholder-[#ffffff33] font-mono text-sm outline-none"
        ...
        {/* Filter pills — also synced with context */}
        <div className="max-w-2xl mx-auto mb-8 hero-animate" style={{ animationDelay: "0.25s" }}>
              ...
              return (
                <button key={cat.id} onClick={() => setActiveFilter(cat.id)}
                  className={[
                    ...
                    "font-mono text-[11px] tracking-[0.15em] uppercase border transition-all duration-200",
                    isActive
        ...
        {/* Carousel */}
        <div className="max-w-5xl mx-auto hero-animate relative" style={{ animationDelay: "0.35s" }}>
          <div className="relative w-full rounded-2xl overflow-hidden border border-[#00f5e022]"
            ...
            {CAROUSEL_SLIDES.map((slide, i) => (
              <div key={slide.src}
                className={["absolute inset-0 transition-all duration-500 ease-in-out",
                  i === current ? "opacity-100 scale-100 z-10" : "opacity-0 scale-[1.02] z-0"].join(" ")}>
                <Image src={slide.src} alt={slide.alt} fill className="object-cover object-center"
                  priority={i === 0} sizes="(max-width: 768px) 100vw, (max-width: 1280px) 90vw, 1024px" />
            ...
            <button onClick={handlePrev} aria-label="Previous slide"
              className="absolute left-3 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full flex items-center justify-center bg-[#080810]/70 backdrop-blur-sm border border-[#00f5e033] text-[#00f5e0] hover:bg-[#00f5e022] hover:border-[#00f5e077] transition-all duration-200 active:scale-90">
              <ChevronIcon direction="left" />
            </button>
            <button onClick={handleNext} aria-label="Next slide"
              className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full flex items-center justify-center bg-[#080810]/70 backdrop-blur-sm border border-[#00f5e033] text-[#00f5e0] hover:bg-[#00f5e022] hover:border-[#00f5e077] transition-all duration-200 active:scale-90">
              <ChevronIcon direction="right" />
            </button>

            <div className="absolute top-0 left-0 right-0 h-px overflow-hidden pointer-events-none z-20">
              <div className="absolute top-0 h-px w-1/3 bg-gradient-to-r from-transparent via-[#00f5e055] to-transparent"
            ...
            {CAROUSEL_SLIDES.map((_, i) => (
              <button key={i} onClick={() => handleDot(i)} aria-label={`Go to slide ${i + 1}`}
                className={["transition-all duration-300 rounded-full",
                  i === current ? "w-6 h-2 bg-[#00f5e0]" : "w-2 h-2 bg-[#00f5e033] hover:bg-[#00f5e066]"].join(" ")}
